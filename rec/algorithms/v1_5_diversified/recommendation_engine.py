...
def apply_cold_start_category_diversity(
    ...
    """
    Apply category diversity to cold start recommendations.
    
    Ensures at least min_per_category episodes from each category
    are included in the top N results, while maintaining quality order.
    
    Args:
        scored: List of ScoredEpisode, already sorted by final_score descending
        config: Algorithm configuration with category diversity settings
        top_n: Number of top recommendations to diversify
    
    Returns:
        Reordered list with category diversity applied
    """
    if not config.cold_start_category_diversity_enabled:
        return scored
    
    if not config.cold_start_categories:
        return scored
    
    min_per_cat = config.cold_start_category_min_per_category
    target_categories = set(config.cold_start_categories)
    
    # Build category -> list of scored episodes (maintaining score order)
    by_category: Dict[str, List["ScoredEpisode"]] = {cat: [] for cat in target_categories}
    ...
    # Select top episodes ensuring category minimums
    ...
    for _ in range(min_per_cat):
        for cat in target_categories:
            if by_category[cat]:
                ep = by_category[cat].pop(0)  # Take highest-scoring from this category
    ...
    # Fill remaining slots with best remaining episodes (any category)
    remaining = []
    ...
    # Re-sort selected by final_score to maintain quality order within the diversified set
    selected.sort(key=lambda x: x.final_score, reverse=True)
    
    # Append remaining episodes (beyond top_n) in original order
...
def rank_candidates(
    ...
    # Apply cold start category diversity if enabled
