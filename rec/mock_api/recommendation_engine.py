...
def get_candidate_pool(
    ...
    if len(candidates) < config.candidate_pool_size // 2:
        if config.freshness_window_days < 60:
            ...
            return get_candidate_pool(excluded_ids, expanded_config, data_cache)
...
def get_user_vector_mean(
    ...
    for eng in sorted_eng:
        ...
        embedding = data_cache.get_embedding(ep_id)
...
def compute_similarity_sum(
    ...
    candidate_embedding = data_cache.get_embedding(candidate["id"])
    ...
    for eng in sorted_eng:
        ep_id = eng.get("episode_id")
        eng_embedding = data_cache.get_embedding(ep_id)
...
def rank_candidates(
    ...
    if not config.use_sum_similarities:
        user_vector = get_user_vector_mean(engagements, config, data_cache)
    ...
    for ep in candidates:
        ...
        if cold_start:
            ...
            sim_score = compute_similarity_sum(ep, engagements, config, data_cache)
        else:
            ep_embedding = data_cache.get_embedding(ep_id)
...
def create_recommendation_queue(
    ...
    # Get candidate pool
    candidates = get_candidate_pool(excluded_ids, config, data_cache)
    ...
    # Rank all candidates
    queue = rank_candidates(engagements, candidates, config, data_cache)
