  ...
  useEffect(() => {
    async function loadEpisodes() {
      try {
        ...
        setError('API not running. Start the backend with: docker-compose up -d');
      } finally {
        setLoading(false);
      }
    }
...
function EpisodeCard({ episode, isViewed, onView, onBookmark }) {
  ...
  return (
    <div
      className={`bg-slate-800 rounded-xl border overflow-hidden transition-colors cursor-pointer ${isViewed
    ...
    >
      <div className="p-4">
        ...
        <div className="flex items-center gap-3 mb-2 text-xs">
          ...
          <span className={`px-2 py-0.5 rounded ${qualityScore >= 6 ? 'bg-green-500/20 text-green-400' :
